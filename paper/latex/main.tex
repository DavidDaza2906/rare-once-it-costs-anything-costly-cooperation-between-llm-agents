...
\begin{center}
  \vspace{0.6em}
  {\LARGE\bfseries Rare Once It Costs Anything: Costly Cooperation Between LLM Agents\footnote{Research
  conducted at the
...
act of abstaining from a shared reserve. We submit to Track 2, What happened and what breaks next
(Appendix~L lists what this offers Tracks 1 and 2).
...
and 4/24 deliveries at prices 5 and 20; no recruiter cell exceeded the plain request. In scenes that
also held a shared reserve, the recruiter request obtained 3/96 and 3/72 deliveries ($K = 5$ and 20),
against 4/48 and 8/48 for a neutral request, so in neither family did a recruiter raise delivery above
the plain request. With 6--16 runs per cell these contrasts cannot rule out effects of $\pm 20$
points.
...
A month of follow-up would turn these bounds into estimates. \emph{(i) A clean instrument:} fix both
harness defects, rotate prices across positions, and randomize what each agent sees, so no post hoc
exclusion is needed. \emph{(ii) The price curve:} about 40 runs at each of prices 0, 1, 5 and 20
(roughly 160 runs, 3--4 minutes and 140k tokens each) would estimate genuine costly cooperation within
about $\pm 4$ points and separate the step at the first unit of cost from the slope after it.
\emph{(iii) The incident-relevant hypothesis:} compare requests framed as help with requests framed as
a step of the target's own task, which our data suggest is what agents actually pay for; this is the
mechanism a recruiter in the July 2026 incident would exploit. \emph{(iv) Generality:} repeat across
frontier and small models, and double-code agents' reasoning. The output would be an open benchmark
with base rates that incident responders can cite.
...
\begin{itemize}[label=--]
  ...
  \url{https://github.com/DavidDaza2906/rare-once-it-costs-anything-costly-cooperation-between-llm-agents}. Harness,
  ...
  \item \textbf{Data explorer:} \url{https://copera-website.onrender.com/}, a website to browse the
  data; its source code is at \url{https://github.com/manueloff14/copera-website}.
